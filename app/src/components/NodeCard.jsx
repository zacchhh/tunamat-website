...
export default function NodeCard({ label, count, active, level = 0, onClick, icon, onQuickAdd, onShowDetail, variant, onCollapse, pillColor, dimmed }) {
  ...
  return (
    <div
      ...
      className={`
        ...
        px-4 flex flex-col justify-center group/card relative
    ...
    >
      ...
      {onCollapse && (
        <button onClick={(e) => { e.stopPropagation(); onCollapse() }}
          className="absolute top-1.5 right-2 w-4 h-4 flex items-center justify-center text-[10px] cursor-pointer border-none bg-transparent rounded-full transition-all opacity-50 hover:opacity-100"
          style={{ color: '#8B84A8' }}
      ...
      {/* "Already added" indicator for dimmed picker nodes */}
      {dimmed && (
        <div className="mt-1">
